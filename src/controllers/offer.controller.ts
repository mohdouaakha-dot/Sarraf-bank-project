...
export const createOffer = async (req: Request, res: Response) => {
  try {
    ...
    const { side, fromCurrency, toCurrency, rate, amountCents, minAmountCents, maxAmountCents, paymentMethod } = req.body;

    if (!side || !fromCurrency || !toCurrency || !rate || !amountCents) {
    ...
    const offer = await prisma.offer.create({
      data: {
        userId, side, fromCurrency, toCurrency,
...
export const cancelOffer = async (req: Request, res: Response) => {
  try {
    ...
    if (offer.userId !== req.user!.id) return res.status(403).json({ error: 'You do not own this offer' });
